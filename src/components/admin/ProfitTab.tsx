      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-1 -mx-1 px-1">
        {sections.map(s => (
          <button key={s.id} onClick={() => setSection(s.id)}
            className={`px-3.5 py-2 rounded-xl text-xs font-medium whitespace-nowrap transition-colors min-h-[38px] ${
        ...
        <div className="space-y-5">
          ...
          <div className="p-5 rounded-2xl bg-card border border-border space-y-4">
            ...
            <div className="flex justify-between text-[11px] text-muted-foreground">
              <span>-30% Sconto</span><span>0%</span><span>+30%</span>
            </div>
              ...
              <motion.div key={review.id}
                ...
                <div className="flex justify-between items-center mb-1 gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-sm font-medium text-foreground truncate">{review.customer_name || "Anonimo"}</span>
                    <span className="text-[10px] text-muted-foreground flex-shrink-0">{new Date(review.created_at).toLocaleDateString("it-IT")}</span>
                  </div>
                ...
                {review.is_public ? (
                  <span className="inline-flex items-center gap-1 text-[10px] text-green-400 mt-1"><ExternalLink className="w-3 h-3" /> Su Google</span>
                ) : (
                  <span className="text-[10px] text-accent mt-1 inline-block">🔒 Archivio privato</span>
                )}
              </motion.div>
            ))}
          </div>
