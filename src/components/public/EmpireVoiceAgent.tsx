...
const SPEECH_START_GUARD_MS = 8000;
const SPEECH_HARD_TIMEOUT_MS = 90000;
const SPEECH_VOICE_WARMUP_RETRIES = 20;
    ...
    if (resp.ok && !abortRef.current) {
      ...
      if (data?.audioContent && !data?.error && !data?.fallback && !abortRef.current) {
        ...
        const played = await new Promise<boolean>((resolve) => {
          ...
          if (abortRef.current) { audio.pause(); resolve(false); return; }
  ...
  const conversation = useConversation({
    ...
    onDisconnect: () => {
      ...
      if (elevenlabsQuotaFailedRef.current) {
        console.log("[Arianna LIVE] 🔄 Quota esaurita — passo alla voce omaggio");
        releaseVoiceAgent("arianna-live");
        setVoiceMode("legacy");
        abortRef.current = false;
        useBrowserFallbackRef.current = true;
        setBrowserOnlyTTS(true);
        setMessages(prev => [...prev, { role: "assistant", content: "Sono passata alla voce omaggio. Continuiamo a parlare! 🎤" }]);
        return;
      }
      ...
      if (!elevenlabsIntentionalStopRef.current) {
        setMessages(prev => [...prev, { role: "assistant", content: "Sono passata alla voce omaggio. Scrivi o parlami, sono qui! 🎤" }]);
      }
    },
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed bottom-0 right-0 z-[9999] w-full sm:w-[380px] sm:bottom-4 sm:right-4 max-h-[70dvh] sm:max-h-[600px] flex flex-col rounded-t-2xl sm:rounded-2xl border border-white/10 bg-[#0b0d12]/95 text-white backdrop-blur-2xl shadow-[0_0_60px_hsla(265,85%,65%,0.25)]"
