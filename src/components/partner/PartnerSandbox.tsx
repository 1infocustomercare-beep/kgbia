...
const PartnerSandbox = () => {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      setDemoLogoUrl(ev.target?.result as string);
    };
          ...
          >
            <div className="p-3 sm:p-4 rounded-xl bg-card border border-border/30 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowCustomizer(false)} className="text-muted-foreground hover:text-foreground">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <p className="text-[10px] text-muted-foreground">
                Inserisci il nome e il logo del ristorante del tuo potenziale cliente per una demo su misura.
              </p>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-xl border-2 border-dashed border-border/50 hover:border-primary/50 flex items-center justify-center transition-all overflow-hidden flex-shrink-0"
                >
                  {demoLogoUrl ? (
                    <img src={demoLogoUrl} alt="Logo" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1 space-y-1.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 rounded-lg bg-background border border-border/40 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50"
              ...
              {(demoRestaurantName !== "Impero Roma" || demoLogoUrl) && (
                <button
                  onClick={() => { setDemoRestaurantName("Impero Roma"); setDemoLogoUrl(null); setDemoAddress("Via del Corso 42, Roma"); setDemoPhone("+39 06 1234567"); setDemoEmail("info@impero.it"); }}
            ...
            <div className="w-full rounded-[26px] sm:rounded-[28px] overflow-hidden bg-background relative">
              ...
              {activeView === "customer" && (
                <div className="h-[500px] sm:h-[560px] overflow-y-auto">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 py-0.5">
                          <div className="flex items-start justify-between gap-1">
                            ...
                            {item.popular && (
                              <span className="px-1 sm:px-1.5 py-0.5 rounded-full bg-primary/10 text-primary text-[6px] sm:text-[7px] font-bold flex-shrink-0">TOP</span>
                            )}
                          </div>
                          ...
                          {priceMultiplier !== 0 && (
                            <span className="text-[8px] sm:text-[9px] text-muted-foreground line-through">€{item.price.toFixed(2)}</span>
                          )}
                        </div>
                      </motion.div>
                    ))}
                  ...
                  <AnimatePresence>
                    {showUpsell && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="p-2.5 sm:p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
                          <Coffee className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400 flex-shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] sm:text-[11px] font-bold text-foreground">Un caffè col dessert? ☕</p>
                            <p className="text-[9px] sm:text-[10px] text-muted-foreground">Prosecco DOC — €{getAdjustedPrice(8).toFixed(2)}</p>
                          </div>
                          <button onClick={() => addToCart(demoMenu[11])}
                            className="px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg bg-primary text-primary-foreground text-[9px] sm:text-[10px] font-bold flex-shrink-0">
                            Aggiungi
                          </button>
                        </div>
                      </motion.div>
                  ...
                  {/* Quick Actions Bar */}
                  <div className="px-3 pb-2 grid grid-cols-3 gap-1 sm:gap-1.5">
                    <button onClick={() => setShowReservation(true)}
                      className="flex flex-col items-center gap-0.5 sm:gap-1 py-2 sm:py-2.5 rounded-xl bg-card border border-border/20 text-center">
                      <CalendarDays className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
                      <span className="text-[8px] sm:text-[9px] font-medium text-muted-foreground">Prenota</span>
                    </button>
                    <button onClick={() => setShowReview(true)}
                      className="flex flex-col items-center gap-0.5 sm:gap-1 py-2 sm:py-2.5 rounded-xl bg-card border border-border/20 text-center">
                      <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
                      <span className="text-[8px] sm:text-[9px] font-medium text-muted-foreground">Recensione</span>
                    </button>
                    <button onClick={() => setShowChat(true)}
                      className="flex flex-col items-center gap-0.5 sm:gap-1 py-2 sm:py-2.5 rounded-xl bg-card border border-border/20 text-center">
                      <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
                      <span className="text-[8px] sm:text-[9px] font-medium text-muted-foreground">Chat</span>
                    </button>
                  </div>
                      ...
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <div onClick={e => e.stopPropagation()}>
                          ...
                          <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3">
                            ...
                            <p className="text-[10px] sm:text-xs text-muted-foreground">Ingredienti freschi di prima qualità, preparazione artigianale con materie prime italiane DOP.</p>
                            <div className="flex items-center justify-between">
                              ...
                              <motion.button onClick={() => { addToCart(selectedItem); setSelectedItem(null); }}
                                className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-primary text-primary-foreground text-xs sm:text-sm font-bold"
                                whileTap={{ scale: 0.95 }}>
                                Aggiungi
                              </motion.button>
                            </div>
                  ...
                  <AnimatePresence>
                    {showReservation && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <div className="p-3 sm:p-4 space-y-3 sm:space-y-4" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm sm:text-base font-display font-bold text-foreground">Prenota un Tavolo</h3>
                            <button onClick={() => setShowReservation(false)} className="text-muted-foreground">✕</button>
                          </div>
                          <div className="space-y-2 sm:space-y-3">
                            <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30">
                              <p className="text-[9px] sm:text-[10px] text-muted-foreground mb-1">Nome</p>
                              <p className="text-xs sm:text-sm text-foreground">Marco Rossi</p>
                            </div>
                            <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                              <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30">
                                <p className="text-[9px] sm:text-[10px] text-muted-foreground mb-1">Data</p>
                                <p className="text-xs sm:text-sm text-foreground">Oggi</p>
                              </div>
                              <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30">
                                <p className="text-[9px] sm:text-[10px] text-muted-foreground mb-1">Ora</p>
                                <p className="text-xs sm:text-sm text-foreground">20:30</p>
                              </div>
                            </div>
                            <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30">
                              <p className="text-[9px] sm:text-[10px] text-muted-foreground mb-1">Ospiti</p>
                              <p className="text-xs sm:text-sm text-foreground">4 persone</p>
                            </div>
                            <button className="w-full py-2.5 sm:py-3 rounded-xl bg-primary text-primary-foreground text-xs sm:text-sm font-bold">
                  ...
                  <AnimatePresence>
                    {showReview && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <div className="p-3 sm:p-4 space-y-3 sm:space-y-4" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm sm:text-base font-display font-bold text-foreground">Lascia una Recensione</h3>
                            <button onClick={() => setShowReview(false)} className="text-muted-foreground">✕</button>
                          </div>
                          <div className="flex justify-center gap-1.5 sm:gap-2 py-2 sm:py-3">
                            {[1, 2, 3, 4, 5].map(s => (
                              <Star key={s} className="w-6 h-6 sm:w-8 sm:h-8 text-primary fill-primary cursor-pointer" />
                            ))}
                          </div>
                          ...
                          <div className="p-2.5 sm:p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-center">
                            ...
                            <p className="text-[9px] sm:text-[10px] font-bold text-foreground">Review Shield™ Attivo</p>
                            <p className="text-[8px] sm:text-[9px] text-muted-foreground">Protegge il rating del ristorante</p>
                          </div>
                        </div>
                      </motion.div>
                    )}
                  ...
                  <AnimatePresence>
                    {showChat && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 flex-1" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm sm:text-base font-display font-bold text-foreground truncate">Chat con {demoRestaurantName}</h3>
                            <button onClick={() => setShowChat(false)} className="text-muted-foreground flex-shrink-0">✕</button>
                          </div>
                          <div className="space-y-2 flex-1">
                            <div className="p-2 sm:p-2.5 rounded-xl bg-card border border-border/20 max-w-[80%]">
                              <p className="text-[10px] sm:text-[11px] text-foreground">Benvenuto! Come possiamo aiutarti?</p>
                              <p className="text-[7px] sm:text-[8px] text-muted-foreground mt-1">14:30</p>
                            </div>
                            <div className="p-2 sm:p-2.5 rounded-xl bg-primary/10 border border-primary/20 max-w-[80%] ml-auto">
                              <p className="text-[10px] sm:text-[11px] text-foreground">Avete tavoli liberi per stasera?</p>
                              <p className="text-[7px] sm:text-[8px] text-muted-foreground mt-1">14:31</p>
                            </div>
                            <div className="p-2 sm:p-2.5 rounded-xl bg-card border border-border/20 max-w-[80%]">
                              <p className="text-[10px] sm:text-[11px] text-foreground">Certo! Per quante persone? Posso prenotarle subito dal menù 🍽️</p>
                              <p className="text-[7px] sm:text-[8px] text-muted-foreground mt-1">14:31</p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <div className="flex-1 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl bg-card border border-border/30 text-[10px] sm:text-xs text-muted-foreground">
                              Scrivi un messaggio...
                            </div>
                            <button className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-primary flex items-center justify-center">
              ...
              {activeView === "admin" && (
                <div className="h-[500px] sm:h-[560px] overflow-y-auto">
                  ...
                  <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-xl border-b border-border/30 px-3 pt-6 sm:pt-7 pb-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <LogoElement size="sm" />
                        <div className="min-w-0">
                          <p className="text-xs sm:text-sm font-display font-bold text-foreground truncate">Admin Dashboard</p>
                          <p className="text-[8px] sm:text-[9px] text-muted-foreground truncate">{demoRestaurantName} — Gestione</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    {/* Admin Sub-tabs */}
                    <div className="flex gap-0.5 sm:gap-1 mt-2 overflow-x-auto no-scrollbar">
                      {([
                        ...
                        <button key={t.id} onClick={() => setAdminTab(t.id)}
                          className={`flex items-center gap-0.5 sm:gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[8px] sm:text-[9px] font-semibold whitespace-nowrap transition-all ${
                  ...
                  <div className="px-3 py-2.5 sm:py-3 space-y-2.5 sm:space-y-3">
                    {/* ---- OVERVIEW TAB ---- */}
                    {adminTab === "overview" && (
                      <>
                        {/* KPIs */}
                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                          {[
                            { label: "Incasso Oggi", value: `€${demoStats.todayRevenue.toFixed(0)}`, icon: <DollarSign className="w-3.5 h-3.5 sm:w-4 sm:h-4" />, color: "text-primary" },
                            { label: "Ordini Oggi", value: demoStats.todayOrders.toString(), icon: <ShoppingCart className="w-3.5 h-3.5 sm:w-4 sm:h-4" />, color: "text-emerald-400" },
                            { label: "Ordini Attivi", value: demoStats.activeOrders.toString(), icon: <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4" />, color: "text-amber-400" },
                            { label: "Rating Medio", value: `${demoStats.avgRating} ★`, icon: <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4" />, color: "text-primary" },
                          ].map((s, i) => (
                            <div key={i} className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30">
                              <div className={`${s.color} mb-0.5 sm:mb-1`}>{s.icon}</div>
                              <p className="text-base sm:text-lg font-display font-bold text-foreground">{s.value}</p>
                              <p className="text-[8px] sm:text-[9px] text-muted-foreground">{s.label}</p>
                            </div>
                          ))}
                        </div>

                        {/* Panic Mode */}
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                              <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-400" />
                              <span className="text-[11px] sm:text-xs font-bold text-foreground">Panic Mode</span>
                            </div>
                            <span className={`text-xs sm:text-sm font-bold ${priceMultiplier > 0 ? "text-emerald-400" : priceMultiplier < 0 ? "text-red-400" : "text-muted-foreground"}`}>
                          ...
                          />
                          <p className="text-[8px] sm:text-[9px] text-muted-foreground text-center">
                            Muovi lo slider → prezzi cambiano <span className="text-primary font-bold">in tempo reale</span>
                          </p>
                        </div>

                        {/* AI-Mary */}
                        <button onClick={() => setActiveProfitCard("ai-mary")}
                          className="w-full flex items-center gap-2 sm:gap-2.5 p-2.5 sm:p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                          <div className="relative flex-shrink-0">
                            <div className="w-3 h-3 rounded-full bg-emerald-400" />
                            <div className="absolute inset-0 w-3 h-3 rounded-full bg-emerald-400 animate-ping opacity-40" />
                          </div>
                          <div className="flex-1 text-left min-w-0">
                            <span className="text-[11px] sm:text-xs font-bold text-foreground">AI-Mary: Fisco 2026 ✓</span>
                            <p className="text-[8px] sm:text-[9px] text-muted-foreground">Registratore collegato, conforme.</p>
                          </div>
                        ...
                        <button onClick={() => { setShowWalletPush(true); setActiveProfitCard("wallet"); setTimeout(() => setShowWalletPush(false), 4000); }}
                          className="w-full py-2.5 sm:py-3 rounded-xl bg-primary/10 border border-primary/30 text-primary text-[10px] sm:text-xs font-semibold flex items-center justify-center gap-2">
                          <Wallet className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Invia Push Wallet ai Clienti
                        </button>

                        {/* Quick stats */}
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 space-y-1.5 sm:space-y-2">
                          <h4 className="text-[11px] sm:text-xs font-bold text-foreground">Oggi in Sintesi</h4>
                    ...
                    {adminTab === "menu" && (
                      <div className="space-y-1.5 sm:space-y-2">
                        <div className="flex items-center justify-between">
                          <h4 className="text-[11px] sm:text-xs font-bold text-foreground">Gestione Menu</h4>
                          <button className="px-2 sm:px-2.5 py-1 rounded-lg bg-primary text-primary-foreground text-[9px] sm:text-[10px] font-bold flex items-center gap-1">
                            <Plus className="w-3 h-3" /> Aggiungi
                          </button>
                        </div>
                        {demoMenu.slice(0, 8).map(item => (
                          <div key={item.id} className="flex items-center gap-2 p-1.5 sm:p-2 rounded-lg bg-card border border-border/20">
                            <img src={item.image} alt={item.name} className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg object-cover" />
                            <div className="flex-1 min-w-0">
                              <p className="text-[9px] sm:text-[10px] font-semibold text-foreground truncate">{item.name}</p>
                              <p className="text-[8px] sm:text-[9px] text-muted-foreground">{item.category}</p>
                            </div>
                            <span className="text-[9px] sm:text-[10px] font-bold text-primary flex-shrink-0">€{getAdjustedPrice(item.price).toFixed(2)}</span>
                            <div className="w-6 sm:w-7 h-3.5 sm:h-4 rounded-full bg-emerald-400/20 flex items-center justify-end px-0.5 flex-shrink-0">
                    ...
                    {adminTab === "orders" && (
                      <div className="space-y-1.5 sm:space-y-2">
                        <h4 className="text-[11px] sm:text-xs font-bold text-foreground">Ordini in Tempo Reale</h4>
                        {kitchenOrders.length === 0 && (
                          <div className="py-6 sm:py-8 text-center">
                            <Package className="w-7 h-7 sm:w-8 sm:h-8 text-muted-foreground/30 mx-auto mb-2" />
                            <p className="text-[10px] sm:text-xs text-muted-foreground">Vai nell'App Cliente, ordina, e l'ordine apparirà qui!</p>
                          </div>
                        )}
                        {[
                          ...
                          <div key={order.id} className={`p-2 sm:p-2.5 rounded-xl border ${
                            ...
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-[9px] sm:text-[10px] font-bold text-foreground">{order.customerName} · Tav.{order.table}</span>
                              <span className={`text-[7px] sm:text-[8px] font-bold px-1.5 py-0.5 rounded-full ${
                            ...
                            <div className="space-y-0.5">
                              {order.items.map((item, i) => (
                                <p key={i} className="text-[8px] sm:text-[9px] text-muted-foreground">{item.qty}x {item.name}</p>
                              ))}
                            </div>
                            <p className="text-[7px] sm:text-[8px] text-muted-foreground mt-1">{order.time} · {order.orderType}</p>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {adminTab === "tables" && (
                      <div className="space-y-2.5 sm:space-y-3">
                        <h4 className="text-[11px] sm:text-xs font-bold text-foreground">Mappa Tavoli</h4>
                        <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
                          {Object.entries(tableStatuses).map(([num, status]) => (
                            <button key={num}
                              ...
                              className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-0.5 sm:gap-1 border text-center transition-all ${
                              ...
                              <span className="text-xs sm:text-sm font-bold text-foreground">{num}</span>
                        ...
                        <div className="flex gap-2 sm:gap-3 justify-center">
                          {[
                            ...
                            <div key={l.label} className="flex items-center gap-1">
                              ...
                              <span className="text-[7px] sm:text-[8px] text-muted-foreground">{l.label}</span>
                            </div>
                          ))}
                        </div>
                        <p className="text-[8px] sm:text-[9px] text-muted-foreground text-center">Tocca un tavolo per cambiare stato</p>
                      </div>
                    )}
                    ...
                    {adminTab === "settings" && (
                      <div className="space-y-2.5 sm:space-y-3">
                        <h4 className="text-[11px] sm:text-xs font-bold text-foreground">Design Studio</h4>
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 space-y-2">
                          <p className="text-[9px] sm:text-[10px] font-semibold text-foreground">Colore Brand</p>
                          <div className="flex gap-1.5 sm:gap-2">
                            {["#C8922A", "#E74C3C", "#2ECC71", "#3498DB", "#9B59B6", "#1A1A2E"].map(c => (
                              <div key={c} className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-border/30 cursor-pointer"
                                style={{ backgroundColor: c }} />
                            ))}
                          </div>
                        </div>
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 space-y-2">
                          <p className="text-[9px] sm:text-[10px] font-semibold text-foreground">QR Code Menu</p>
                          <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto bg-foreground/5 rounded-xl flex items-center justify-center">
                            <QrCode className="w-10 h-10 sm:w-12 sm:h-12 text-foreground/20" />
                          </div>
                          <p className="text-[8px] sm:text-[9px] text-muted-foreground text-center">Scarica e stampa per ogni tavolo</p>
                        </div>
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 space-y-1">
                          <p className="text-[9px] sm:text-[10px] font-semibold text-foreground">Orari di Apertura</p>
                          {["Lun-Ven: 12:00-15:00 · 19:00-23:30", "Sabato: 12:00-24:00", "Domenica: Chiuso"].map((h, i) => (
                            <p key={i} className="text-[8px] sm:text-[9px] text-muted-foreground">{h}</p>
                          ))}
                        </div>
                        <div className="p-2.5 sm:p-3 rounded-xl bg-card border border-border/30 flex items-center gap-2">
                          <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary flex-shrink-0" />
                          <div className="min-w-0">
                            <p className="text-[9px] sm:text-[10px] font-semibold text-foreground">Contatti</p>
                            <p className="text-[8px] sm:text-[9px] text-muted-foreground truncate">{demoPhone} · {demoEmail}</p>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                <div className="h-[500px] sm:h-[560px] overflow-y-auto">
                  ...
                  <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-xl border-b border-border/30 px-3 pt-6 sm:pt-7 pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <div className="px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-[8px] sm:text-[9px] font-bold flex items-center gap-1">
                          <CircleDot className="w-2 h-2 sm:w-2.5 sm:h-2.5" /> LIVE
                        </div>
                      </div>
                    </div>
                    {/* Kitchen Stats */}
                    <div className="flex gap-1.5 sm:gap-2 mt-2">
                      {[
                        ...
                        <div key={s.label} className={`flex-1 py-1 sm:py-1.5 rounded-lg border text-center ${s.color}`}>
                          <p className="text-sm sm:text-base font-bold">{s.count}</p>
                          <p className="text-[7px] sm:text-[8px] font-semibold">{s.label}</p>
                        </div>
                      ))}
                    </div>
                  </div>

                  <div className="px-3 py-2.5 sm:py-3 space-y-1.5 sm:space-y-2">
                    {[
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                            {order.status === "new" && (
                              <motion.div animate={{ scale: [1, 1.3, 1] }} transition={{ repeat: Infinity, duration: 1 }}>
                                <Volume2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-red-400 flex-shrink-0" />
                              </motion.div>
                            )}
                            <span className="text-[10px] sm:text-xs font-bold text-foreground truncate">{order.customerName}</span>
                            <span className="text-[7px] sm:text-[8px] text-muted-foreground px-1 sm:px-1.5 py-0.5 rounded bg-muted flex-shrink-0">Tav.{order.table}</span>
                          </div>
                          <span className="text-[7px] sm:text-[8px] text-muted-foreground flex-shrink-0">{order.time}</span>
                        </div>
                        ...
                        <div className="space-y-1 sm:space-y-1.5 mb-2 sm:mb-2.5">
                          {order.items.map((item, i) => (
                            <div key={i} className="flex items-center gap-1.5 sm:gap-2">
                              <img src={item.image} alt={item.name} className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg object-cover flex-shrink-0" />
                              <span className="text-[10px] sm:text-[11px] text-foreground font-medium truncate">{item.qty}x {item.name}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="flex gap-1 sm:gap-1.5">
                          {order.status === "new" && (
                            <button onClick={() => updateOrderStatus(order.id, "preparing")}
                              className="flex-1 py-1.5 sm:py-2 rounded-lg bg-amber-500/20 text-amber-400 text-[9px] sm:text-[10px] font-bold flex items-center justify-center gap-1">
                              <CookingPot className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> In Prep.
                            </button>
                          )}
                          {order.status === "preparing" && (
                            <button onClick={() => updateOrderStatus(order.id, "ready")}
                              className="flex-1 py-1.5 sm:py-2 rounded-lg bg-emerald-500/20 text-emerald-400 text-[9px] sm:text-[10px] font-bold flex items-center justify-center gap-1">
                              <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Pronto!
                            </button>
                          )}
                          {order.status === "ready" && (
                            <button onClick={() => updateOrderStatus(order.id, "delivered")}
                              className="flex-1 py-1.5 sm:py-2 rounded-lg bg-muted text-muted-foreground text-[9px] sm:text-[10px] font-bold flex items-center justify-center gap-1">
                              <UserCheck className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Consegnato
                            </button>
                          )}
                        </div>
                      </motion.div>
              ...
              {activeView === "customer" && cartCount > 0 && !selectedItem && !showReservation && !showReview && !showChat && (
                ...
                >
                  <button onClick={placeOrder}
                    className="w-full py-2.5 sm:py-3 rounded-xl bg-primary text-primary-foreground font-bold text-[10px] sm:text-xs flex items-center justify-center gap-2 shadow-lg shadow-primary/30">
