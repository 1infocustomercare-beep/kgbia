  ...
  useEffect(() => {
    ...
    class Node {
      ...
      draw() {
        ...
        ctx!.beginPath();
        ctx!.arc(this.x, this.y, this.r * s, 0, 6.28);
        ctx!.fillStyle = "rgba(108,60,224,0.3)";
        ctx!.fill();
      }
    }
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < 180) {
            ...
            ctx.strokeStyle = `rgba(108,60,224,${0.05 * (1 - d / 180)})`;
            ctx.lineWidth = 0.7;
            ctx.stroke();
          }
        }
...
export default function LandingAgents() {
  return (
    <section id="agenti" className="relative overflow-hidden py-16 lg:py-24" style={{ background: "linear-gradient(180deg, #080810, #111128, #080810)" }}>
      <NeuralCanvas />
      <div className="relative z-[1] max-w-[1320px] mx-auto px-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          {AGENTS.map((a, i) => (
            ...
            >
              <LandingPremiumPanel
                eyebrow={a.panelEyebrow}
                code={a.panelCode}
                title={a.name}
                tone={a.tagColor === "gold" ? "gold" : a.tagColor === "purple" ? "violet" : "teal"}
              />
              ...
              <span
                className={`inline-block text-[10px] px-2.5 py-1 rounded-lg font-semibold ${
                  a.tagColor === "teal"
                    ? "bg-[rgba(126,183,190,0.12)] text-[#7eb7be]"
                    : a.tagColor === "gold"
                      ? "bg-[rgba(212,168,85,0.12)] text-[#d4a855]"
