...
export default function LandingHero() {
  ...
  // Progress for mockup fan reveal — mapped to scroll
  const fanProgress = useTransform(scrollYProgress, [0.05, 0.55], [0, 1]);
  const titleOpacity = useTransform(scrollYProgress, [0, 0.1], [1, 1]);
  const titleScale = useTransform(scrollYProgress, [0.4, 0.7], [1, 0.92]);

  return (
    <section ref={sectionRef} id="hero" className="relative" style={{ height: "250vh" }}>
      ...
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col">
        ...
        <motion.div className="absolute w-[600px] h-[600px] rounded-full blur-[160px] pointer-events-none z-[1]"
          style={{ background: "radial-gradient(circle, rgba(126,183,190,0.15), transparent 70%)", top: "5%", right: "-10%" }}
          animate={{ scale: [1, 1.08, 1], opacity: [0.8, 1, 0.8] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute w-[500px] h-[500px] rounded-full blur-[140px] pointer-events-none z-[1]"
          style={{ background: "radial-gradient(circle, rgba(108,60,224,0.12), transparent 70%)", bottom: "10%", left: "-8%" }}
          animate={{ scale: [1, 1.06, 1] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }} />

        {/* Section background */}
        <div className="absolute inset-0 z-0" style={{
          background: "radial-gradient(ellipse 80% 60% at 50% 20%, rgba(13,13,30,0.9) 0%, #020204 70%)",
        }} />

        {/* MAIN CONTENT */}
        <div className="relative z-[2] flex-1 flex flex-col justify-center items-center pt-[120px] pb-4 px-5">
          <motion.div style={{ opacity: titleOpacity, scale: titleScale }} className="text-center max-w-[900px] mx-auto mb-6 lg:mb-10">
            {/* Badge */}
            <motion.div
            ...
            >
              <motion.span
                className="block text-white drop-shadow-[0_2px_20px_rgba(255,255,255,0.08)]"
              ...
              <motion.span
                className="block bg-gradient-to-r from-[#7eb7be] via-[#9b8ade] to-[#6c3ce0] bg-clip-text text-transparent drop-shadow-[0_2px_30px_rgba(126,183,190,0.2)]"
            ...
            {/* Typing subtitle */}
            <motion.p
              className="text-white/60 text-[clamp(1rem,1.8vw,1.25rem)] leading-[1.7] max-w-[600px] mx-auto mb-8 font-light"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {TRUST.map((t) => (
                <span key={t} className="text-[11px] text-white/35 font-medium tracking-wide flex items-center gap-1.5">
          ...
          {/* SCROLL-DRIVEN MOCKUP FAN */}
          <div className="relative w-full max-w-[1100px] mx-auto">
            <div className="relative flex justify-center items-end" style={{ height: isMobile ? "220px" : "360px", perspective: "1800px" }}>
              {MOCKUPS.map((m, i) => {
            ...
            {/* Reflection gradient */}
            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#020204] to-transparent z-[20] pointer-events-none" />
          </div>
        </div>
        ...
        >
          <span className="text-[9px] tracking-[3px] uppercase text-white/20 font-semibold">Scorri per scoprire</span>
          <motion.div
            className="w-[1px] h-7"
...
/* Scroll-driven individual phone */
function MockupPhone({
  m, i, offset, total, center, spreadX, spreadY, rot, phoneW, progress,
}: {
  m: { src: string; label: string; cat: string };
  i: number; offset: number; total: number; center: number;
  spreadX: number; spreadY: number; rot: number; phoneW: number;
  progress: any;
}) {
  const delay = Math.abs(offset) * 0.12;
  const itemStart = Math.min(delay, 0.6);
  const itemEnd = Math.min(delay + 0.5, 1);

  const y = useTransform(progress, [itemStart, itemEnd], [250, -Math.abs(offset) * spreadY]);
  const x = useTransform(progress, [itemStart, itemEnd], [0, offset * spreadX]);
  const opacity = useTransform(progress, [itemStart, itemStart + 0.15], [0, 1]);
  const scale = useTransform(progress, [itemStart, itemEnd], [0.7, 1 - Math.abs(offset) * 0.03]);
  const rotate = useTransform(progress, [itemStart, itemEnd], [rot * 2.5, rot]);
  ...
  return (
    ...
    >
      ...
      <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-center">
        ...
        <p className="text-[8px] text-white/40">{m.cat}</p>
      </div>
    </motion.div>
  );
}
