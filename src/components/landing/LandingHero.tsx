...
function MockupPhone({ m, i, total, progress, isMobile }: {
  ...
  // Each phone fans out based on scroll progress (0→1)
  // Stagger: center phones appear first
  const stagger = Math.abs(offset) * 0.08;
  const localP = Math.max(0, Math.min(1, (progress - stagger) / (1 - stagger)));
  const eased = 1 - Math.pow(1 - localP, 3); // easeOutCubic

  const x = eased * offset * spreadX;
  const y = 200 * (1 - eased) + (-Math.abs(offset) * spreadY * eased);
  const opacity = Math.min(1, localP * 2.5);
  const scale = 0.6 + eased * (0.4 - Math.abs(offset) * 0.03);
...
export default function LandingHero() {
  ...
  // This section is 300vh tall — the sticky inner stays pinned for 200vh of scroll
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end end"],
  });

  // Phase 1: 0-0.15 = title cinematic entrance
  // Phase 2: 0.15-0.6 = mockups fan out
  // Phase 3: 0.6-1.0 = hold + fade out as user scrolls past
  const titleScale = useTransform(scrollYProgress, [0, 0.12, 0.5, 0.85], [0.85, 1, 1, 0.92]);
  const titleOpacity = useTransform(scrollYProgress, [0, 0.08, 0.7, 0.95], [0.3, 1, 1, 0]);
  const titleZ = useTransform(scrollYProgress, [0, 0.15], [80, 0]);
  const titleBlur = useTransform(scrollYProgress, [0, 0.12], [6, 0]);
  const badgeOpacity = useTransform(scrollYProgress, [0.03, 0.1], [0, 1]);
  ...
  useMotionValueEvent(scrollYProgress, "change", (v) => {
    const p = Math.max(0, Math.min(1, (v - 0.12) / 0.43));
    setFanProgress(p);
  });
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        <motion.div className="absolute inset-0" style={{ opacity: sectionFadeOut }}>
          {/* Background */}
          ...
          {/* Ambient glows */}
          <div className="absolute w-[600px] h-[600px] rounded-full blur-[160px] pointer-events-none z-[1] animate-pulse"
          ...
          {/* CONTENT */}
          <div className="relative z-[2] h-full flex flex-col justify-center items-center px-5">
            {/* Title block with cinematic depth effect */}
            <motion.div
              className="text-center max-w-[900px] mx-auto mb-6 lg:mb-10"
              style={{
                scale: titleScale,
            ...
            >
              <motion.div style={{ translateZ: titleZ, filter: useTransform(titleBlur, v => `blur(${v}px)`) }}>
                ...
                {/* Headline */}
                <h1 className="text-[clamp(2.6rem,6vw,5.8rem)] font-heading font-extrabold leading-[0.92] tracking-[-0.02em] mb-6">
                  <span className="block text-white drop-shadow-[0_2px_20px_rgba(255,255,255,0.1)]">
                    Il Tuo Business.
                  </span>
                  <span className="block bg-gradient-to-r from-[#7eb7be] via-[#9b8ade] to-[#6c3ce0] bg-clip-text text-transparent drop-shadow-[0_2px_30px_rgba(126,183,190,0.25)]">
                    Completamente Automatizzato.
                  </span>
                </h1>

                {/* Subtitle */}
                <p className="text-white/65 text-[clamp(1rem,1.8vw,1.25rem)] leading-[1.7] max-w-[600px] mx-auto mb-8 font-light">
                  {typed}
            ...
            {/* MOCKUP FAN — driven by scroll progress, fully reversible */}
            <div className="relative w-full max-w-[1100px] mx-auto">
              <div
                className="relative flex justify-center items-end"
                style={{ height: isMobile ? "220px" : "380px", perspective: "1800px" }}
              >
                {MOCKUPS.map((m, i) => (
                  <MockupPhone
                    key={i}
