...
import { Star, Globe, Phone, Mail, MapPin, TrendingUp, Eye, MessageSquare, BookmarkPlus } from "lucide-react";
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            {lead.phone && <span className="flex items-center gap-1 text-[10px]" style={{ color: "#6b7280" }}><Phone className="w-3 h-3" /> {lead.phone}</span>}
            {lead.email && <span className="flex items-center gap-1 text-[10px]" style={{ color: "#6b7280" }}><Mail className="w-3 h-3" /> {lead.email}</span>}
        ...
        <div className="flex flex-col items-center flex-shrink-0">
          ...
          <span className="text-[8px] mt-1 font-semibold uppercase tracking-wider" style={{ color: scoreColor }}>Score</span>
        </div>
      </div>

      {/* Actions */}
      <div className="flex gap-2 mt-3">
        <button onClick={() => onAnalyze(lead)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[10px] font-semibold transition-all" style={{ background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.2)", color: "#a78bfa" }}>
          <Eye className="w-3 h-3" /> Analizza
        </button>
        <button onClick={() => onMessage(lead)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[10px] font-semibold transition-all" style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.2)", color: "#10b981" }}>
          <MessageSquare className="w-3 h-3" /> Messaggio
        </button>
        <button onClick={() => onSave(lead)} className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-[10px] font-semibold transition-all" style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)", color: "#3b82f6" }}>
