...
const SpecularPanel = ({ gx, gy, delay, children, className = "" }: {
  ...
  const reflectX = useSpring(useTransform(gx, (v: number) => v * 120), { stiffness: 80, damping: 20 });
  const reflectY = useSpring(useTransform(gy, (v: number) => v * 120), { stiffness: 80, damping: 20 });
  const tiltX = useSpring(useTransform(gy, (v: number) => v * -4), { stiffness: 120, damping: 25 });
  const tiltY = useSpring(useTransform(gx, (v: number) => v * 4), { stiffness: 120, damping: 25 });
...
const Orb = ({ color, size, x, y, gx, gy, parallaxFactor }: {
  ...
  const px = useSpring(useTransform(gx, (v: number) => v * 30 * parallaxFactor), { stiffness: 50, damping: 30 });
  const py = useSpring(useTransform(gy, (v: number) => v * 30 * parallaxFactor), { stiffness: 50, damping: 30 });
  return (
    <motion.div
