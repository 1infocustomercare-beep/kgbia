...
const EmpireDNABackground = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // ═══ LAYER 0: DNA Helix Strands (rotating, scroll-aware) ═══
      const helixRotation = time * 0.5 + scrollNorm * Math.PI * 2;
      const helixAmp = w * 0.12;
      const helixCx = w * 0.5;

      for (let s = 0; s < DNA_STRANDS; s++) {
        const phaseOffset = (s / DNA_STRANDS) * Math.PI * 2;

        // Main strand path
        ctx.beginPath();
        for (let i = 0; i <= DNA_SEGMENTS; i++) {
          const frac = i / DNA_SEGMENTS;
          const y = h * 0.02 + frac * h * 0.96;
          const phase = frac * Math.PI * 8 + helixRotation + phaseOffset;
          const depth = Math.cos(phase) * 0.3 + 0.7;
          const x = helixCx + Math.sin(phase) * helixAmp * depth;
          ...
          if (ptr.active) {
            ...
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < repelRadius * 1.5 && dist > 1) {
              const force = (1 - dist / (repelRadius * 1.5)) * 20;
              fx += (dx / dist) * force;
              fy += (dy / dist) * force;
            }
          }
        ...
        ctx.strokeStyle = hsla(palDna, palDna[3] * 1.8);
        ctx.lineWidth = 1.2;
        ctx.stroke();
        ...
        ctx.strokeStyle = hsla(palDna, palDna[3] * 0.3);
        ctx.lineWidth = 5;
        ctx.stroke();
      }

      // DNA cross-links (base pairs)
      const rungCount = IS_MOBILE ? 18 : 35;
      ctx.lineWidth = 0.4;
      for (let i = 0; i < rungCount; i++) {
        const frac = i / rungCount;
        const y = h * 0.02 + frac * h * 0.96;
        const phase = frac * Math.PI * 8 + helixRotation;
        const x1 = helixCx + Math.sin(phase) * helixAmp * (Math.cos(phase) * 0.3 + 0.7);
        const x2 = helixCx + Math.sin(phase + Math.PI * 2 / DNA_STRANDS) * helixAmp * (Math.cos(phase + Math.PI * 2 / DNA_STRANDS) * 0.3 + 0.7);
        const rungAlpha = palDna[3] * 0.8 * (0.4 + 0.6 * Math.sin(frac * Math.PI * 4 + time));
        ctx.strokeStyle = hsla(palDna, rungAlpha);
        ctx.beginPath();
        ctx.moveTo(x1, y);
        ctx.lineTo(x2, y);
        ctx.stroke();
      ...
      // DNA energy particles flowing along strands
      const energyCount = IS_MOBILE ? 6 : 12;
      for (let i = 0; i < energyCount; i++) {
        const strand = i % DNA_STRANDS;
        const phaseOffset = (strand / DNA_STRANDS) * Math.PI * 2;
        const speed = 0.08 + (i % 4) * 0.03;
        const frac = ((time * speed + i * 0.17) % 1);
        const y = h * 0.02 + frac * h * 0.96;
        const phase = frac * Math.PI * 8 + helixRotation + phaseOffset;
        const depth = Math.cos(phase) * 0.3 + 0.7;
        const x = helixCx + Math.sin(phase) * helixAmp * depth;
        const pa = Math.sin(frac * Math.PI) * 0.8;

        const grad = ctx.createRadialGradient(x, y, 0, x, y, IS_MOBILE ? 5 : 8);
        grad.addColorStop(0, hsla(palPulse, pa * 0.5));
        grad.addColorStop(1, hsla(palPulse, 0));
        ctx.beginPath();
        ctx.arc(x, y, IS_MOBILE ? 5 : 8, 0, Math.PI * 2);
        ctx.fillStyle = grad;
        ctx.fill();

        ctx.beginPath();
        ctx.arc(x, y, IS_MOBILE ? 1.5 : 1.2, 0, Math.PI * 2);
        ctx.fillStyle = hsla(palPulse, pa * 0.9);
        ctx.fill();
      }
