...
export default function ClientsPage() {
  ...
  const { data: clients = [], isLoading } = useQuery({
    queryKey: ["crm_clients", companyId],
    enabled: !!companyId,
    queryFn: async () => {
  ...
  const filtered = clients.filter((c: any) => `${c.first_name} ${c.last_name}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between">
        <h1 className="text-xl sm:text-2xl font-bold font-heading">👥 Anagrafica Clienti</h1>
