...
export default function SettingsPage() {
  ...
  const saveCompany = async () => {
    if (!companyId) return;
    // Try companies table first, fallback to restaurants
    ...
    if (error) {
      // Fallback: try restaurants table
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold font-heading">Impostazioni</h1>
      ...
      <Tabs defaultValue="company">
        ...
        <TabsContent value="company" className="mt-6">
          <Card className="glass border-border/50">
            ...
            <CardContent className="space-y-4">
              <div><Label>Nome Azienda</Label><Input value={companyForm.name} onChange={(e) => setCompanyForm({ ...companyForm, name: e.target.value })} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Telefono</Label><Input value={companyForm.phone} onChange={(e) => setCompanyForm({ ...companyForm, phone: e.target.value })} /></div>
                <div><Label>Email</Label><Input value={companyForm.email} onChange={(e) => setCompanyForm({ ...companyForm, email: e.target.value })} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Indirizzo</Label><Input value={companyForm.address} onChange={(e) => setCompanyForm({ ...companyForm, address: e.target.value })} /></div>
                <div><Label>Città</Label><Input value={companyForm.city} onChange={(e) => setCompanyForm({ ...companyForm, city: e.target.value })} /></div>
              </div>
              <Button className="bg-vibrant-gradient text-white" onClick={saveCompany}>Salva</Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="profile" className="mt-6">
          <Card className="glass border-border/50">
            ...
            <CardContent className="space-y-4">
              <div><Label>Nome Completo</Label><Input value={profileForm.full_name} onChange={(e) => setProfileForm({ ...profileForm, full_name: e.target.value })} /></div>
              <div><Label>Email</Label><Input value={user?.email || ""} disabled className="opacity-60" /></div>
              <Button className="bg-vibrant-gradient text-white" onClick={saveProfile}>Salva Profilo</Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="branding" className="mt-6">
          <Card className="glass border-border/50">
            ...
            <CardContent className="space-y-4">
              ...
              <Button className="bg-vibrant-gradient text-white" onClick={saveBranding}>Salva Branding</Button>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
