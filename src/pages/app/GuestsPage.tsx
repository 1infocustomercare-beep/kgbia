...
export default function GuestsPage() {
  ...
  const filtered = guests.filter((g: any) => `${g.first_name} ${g.last_name}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between">
        <h1 className="text-xl sm:text-2xl font-bold font-heading">🛎️ Registro Ospiti</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="h-11"><Plus className="w-4 h-4 mr-2" /> Aggiungi</Button></DialogTrigger>
          <DialogContent><DialogHeader><DialogTitle>Nuovo Ospite</DialogTitle></DialogHeader>
            <div className="space-y-4 pt-4">
              ...
              <Button className="w-full h-11" onClick={() => addGuest.mutate()} disabled={!form.first_name}>Aggiungi</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>

      <div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" /><Input placeholder="Cerca ospite..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-11" /></div>

      <div className="space-y-2">
        {filtered.map((g: any) => (
          <Card key={g.id}><CardContent className="p-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center"><User className="w-5 h-5 text-primary" /></div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold truncate">{g.first_name} {g.last_name || ""}</p>
      ...
      {filtered.length === 0 && <Card className="border-dashed"><CardContent className="p-8 text-center text-muted-foreground">Nessun ospite registrato.</CardContent></Card>}
    </div>
  );
}
